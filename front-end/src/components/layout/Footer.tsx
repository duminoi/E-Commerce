    ...
    <footer className="bg-on-surface w-full">
      ...
      <div className="max-w-max_width mx-auto px-gutter pt-[64px] pb-[48px]">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-xl">
          ...
          <div className="col-span-1">
            <div
              className="text-[22px] font-bold tracking-tighter text-surface-bright mb-md font-heading"
            >
              LUXE
            </div>
            ...
            <div className="flex items-center gap-sm">
              {['instagram', 'facebook', 'twitter'].map((platform) => (
                <a
                  ...
                  href="#"
                  aria-label={platform}
                  className="w-9 h-9 rounded-full flex items-center justify-center border border-outline-variant/30 text-outline-variant hover:text-surface-bright hover:border-outline-variant transition-all duration-200"
                >
                  <span className="material-symbols-outlined text-[18px]">
                    {platform === 'instagram' ? 'photo_camera' : platform === 'facebook' ? 'thumb_up' : 'share'}
                  </span>
                </a>
              ))}
            </div>
              ...
              <ul className="space-y-[12px]">
                {['About Us', 'Careers', 'Press', 'Contact'].map((item) => (
                  <li key={item}>
                    <a
                      href="#"
                      className="text-[14px] text-outline-variant hover:text-surface-bright transition-colors font-body"
                    >
                      {item}
                    </a>
                  </li>
      ...
      <div className="border-t border-on-surface-variant/20">
        <div className="max-w-max_width mx-auto px-gutter py-[24px] flex flex-col sm:flex-row items-center justify-between gap-sm">
          <p
            className="text-[13px] text-outline-variant font-body"
          >
          ...
          <div className="flex items-center gap-sm">
            {['Visa', 'MC', 'PayPal', 'AMEX'].map((method) => (
              <span
                key={method}
                className="px-2 py-1 bg-on-surface-variant/20 rounded text-[11px] font-semibold text-outline-variant"
              >
                {method}
              </span>
            ))}
